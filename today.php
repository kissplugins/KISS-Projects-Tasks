...
function ptt_render_today_page_html() {
       ...
       <div class="wrap" id="ptt-today-page-container">
               ...
               <div class="ptt-today-user-selector">
                       <?php
                       wp_dropdown_users( [
                               'name'             => 'ptt-today-user-select',
                               'id'               => 'ptt-today-user-select',
                               'selected'         => get_current_user_id(),
                               'show_option_none' => __( 'Select User', 'ptt' ),
                       ] );
                       ?>
               </div>
               ...
               <div class="ptt-today-entry-box">
                       <div class="ptt-today-input-group">
                               ...
                               <select id="ptt-today-task-select" disabled>
                                       <option value=""><?php esc_html_e( '-- Select a Task --', 'ptt' ); ?></option>
                               </select>
                               <input type="text" id="ptt-today-project-display" readonly placeholder="<?php esc_attr_e( 'Project', 'ptt' ); ?>">
                       </div>
 ...
 * Fetches tasks for a specific user that are "Not Started" or "In Progress" and sorts them by last modified.
 */
	...
	if ( empty( $status_terms_to_include ) ) {
       ...
       if ( $query->have_posts() ) {
               while ( $query->have_posts() ) {
                       $query->the_post();
                      $project_terms = get_the_terms( get_the_ID(), 'project' );
...
function ptt_today_start_new_session_callback() {
	...
	$new_session = [
       ...
       // Ensure task status is set to "In Progress" when a timer starts.
...
/**
 * AJAX handler to get time entries for a specific day and user.
 */
...
function ptt_get_active_session_index_for_user( $user_id ) {
       $args = [
               ...
               'meta_query'     => [
                       [
                               'key'     => 'ptt_assignee', // Check tasks assigned to the user
                               'value'   => $user_id,
                               'compare' => '=',
                       ],
               ],
